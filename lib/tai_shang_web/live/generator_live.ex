defmodule TaiShangWeb.GeneratorLive do
  ...
  @impl true
  def handle_event("change_v", %{"_target" => ["erc721_addr"], "erc721_addr" => erc721_addr}, socket) do
    {:noreply,
      socket
  ...
  def handle_event("change_v", %{"_target" => ["evidence_addr"], "evidence_addr" => evidence_addr}, socket) do
    {:noreply,
      socket
  ...
  def handle_event(
    ...
    {token_id, uri} =
      if existed_token_id == "" do
        ...
        uri =
          NFTPlusInteractor.token_uri(erc_721_addr, String.to_integer(existed_token_id))
        Process.send_after(self(), :add_extra_parents, 1000)
